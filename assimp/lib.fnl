...
(fn parse-def-value [file-content define-name default-val]
...
(local assimp-conf-const [(fn [file-content]
                            (let [name "AI_MAX_NUMBER_OF_TEXTURECOORDS"
                                  parsed (parse-def-value file-content name 8)]
                              (values name parsed)))
                          (fn [file-content]
                            (let [name "AI_MAX_NUMBER_OF_COLOR_SETS"
                                  parsed (parse-def-value file-content name 8)]
                              (values name parsed)))])
...
(fn gen-header-const [file-content]
  (var header "")
  (local vars {})
  (each [_i parser (ipairs assimp-conf-const)]
    (let [(name value) (parser file-content)
          val-str (string.format "static const unsigned int %s = %d;\n" name
                                 value)]
      (set header (.. header val-str))
      (set (. vars name) value)))
  (values header vars))

(fn gen-header-defs [file-content]
  (var header "")
  (each [_i parser (ipairs assimp-conf-defs)]
    (set header (.. header (parser file-content))))
  header)

(fn parse-assimp-header [include-path]
  (var header "")
  (let [path (.. include-path "config.h")
        file (assert (io.open path "r") "Assimp header %s not found" path)
        file-content (file:read "*all")
        header-def (gen-header-defs file-content)
        (header-const header-vars) (gen-header-const file-content)]
    (set header (.. header header-def header-const))
    (values header header-vars)))

;; TODO: Don't hardcode the include folder?
(local (cdef-header assimp-defs) (parse-assimp-header "/usr/include/assimp/"))

;; ASSIMP 5.2.5 C interface
(ffi.cdef cdef-header)
(ffi.cdef "
...
{:lib (ffi.load :assimp) :defs assimp-defs}
